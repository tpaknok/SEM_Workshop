...
**1) Y increases with Z1 and 2) Z1 and Z2 increases with X.**
...
This model is examining the direct effect of each variable. The number here is standardized effect size (i.e. in the unit of 1 SD), thus they don't match the regression table output, which present unstandarized slope.
...
```
## Mini task
**Try comparing the multiple regression and SEM results! What are the similarities and differences between two approaches?**
...
I understand a lot of reviewers will not trust your model if any goodness-of-fit test shows p < 0.05. I never think this is a problem, unless you want to build a predictive model. However, I don't want to argue. If you want, you can always add missing link until p > 0.05 for both goodness-of-fit test.
...
## Mini task
Let's build another model, without the link between X and Z1! The result shoud be something like this

```{r,echo=FALSE}
SEM3 <- psem(m6,
             m4,
             Z2 %~~% Z1)
summary(SEM3,conserve=T,.progressBar = F)
plot(SEM3)
```

## Model selection
Actually, if you add some links recommended by psem, this is also model selection. Model selection is a very broad topic and doesn't really have a standardized strategy. In piecewiseSEM you can also use AIC to evaluate different models
SEM3 (the reduced model) has a lower AIC than SEM2. But we know that the link between Z1 and X is correct. What's happening???? \

Turns out that the AIC of the SEM function is merely summing all AICs of the component models. For example, SEM4 has m4 and m6...so this is just the sum of AIC(m4) and AIC(m6).
...
**Note that given how the AIC is calculated, it might make more sense to compare AIC of SEMs with the same number of component models (and of course they should have the same responses). If you remove a component model, I don't think it is a fair comparison.**
...
**All SEMs should start with some theories. How should vegetation, fire and topography interact? What's the direct and indirect effect to each other? Think about it, before you construct your model**
